...
public class GetEmployeeDetailsServiceImpl implements IGetEmployeeDetails {
	...
	List<Employee> list = Arrays.asList(new Employee(100, "Gourav", "Arya", "Male", "UP", "Banking", 120000),
	...
	public List<Employee> getEmpWhoIsHavingMaxSalary() {
		return list.stream().sorted(Comparator.comparingDouble(Employee::getSalary).reversed()).skip(0).limit(1).collect(Collectors.toList());
	}
